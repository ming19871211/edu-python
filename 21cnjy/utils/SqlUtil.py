...
class Mysql(object):
    ...
    #连接池对象
    __pool = None
    def __init__(self, creator=MySQLdb, mincached=DBCFG.mincached, maxcached=DBCFG.maxcached,
                 maxconnections=DBCFG.maxconnections,host=DBCFG.host, port=DBCFG.port, user=DBCFG.user,
                 passwd=DBCFG.passwd, db=DBCFG.db, use_unicode=False, charset=DBCFG.charset):
        ...
        self.__conn = self.__getConn(self)
    ...
    def __getConn(self):
        ...
        if Mysql.__pool is None:
            __pool = PooledDB(creator=self.creator, mincached=self.mincached, maxcached=self.maxcached,
                              maxconnections=self.maxconnections, host=self.host, port=self.port,user=self.user,
                              passwd=self.passwd, db=self.db, use_unicode=self.use_unicode, charset=self.charset)
